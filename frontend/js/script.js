...
function renderizarOpcoesFundo() {
    ...
    opcoesFundoLeitura.forEach((opcao) => {
        ...
        botao.type = 'button';
        botao.className = 'background-option';
        botao.dataset.fundoId = opcao.id;
        botao.setAttribute('aria-pressed', String(opcao.id === fundoSelecionadoId));

        const miniatura = opcao.origem
            ? `<img class="background-option-thumb" src="${opcao.origem}" alt="" loading="lazy">`
            : `<div class="background-option-thumb background-option-thumb-auto" aria-hidden="true"><span>↻</span></div>`;

        botao.innerHTML = `
            ${miniatura}
            <div class="background-option-name">${opcao.nome}</div>
        `;
...
function aplicarModoFantasma(estado) {
    document.body.classList.remove('ghost-mode', 'ghost-mode-30', 'ghost-mode-transparent');

    if (estado === '30') {
        document.body.classList.add('ghost-mode', 'ghost-mode-30');
    } else if (estado === 'transparent') {
        document.body.classList.add('ghost-mode', 'ghost-mode-transparent');
    }
    ...
    if (botao) {
        ...
        botao.textContent = icones[estado] || icones.off;
    }
}
...
const estadosGhostMode = ['off', '30', 'transparent'];
const ghostSalvo = localStorage.getItem('ghostmode');
const ghostEstadoAtual = estadosGhostMode.includes(ghostSalvo) ? ghostSalvo : 'off';
aplicarModoFantasma(ghostEstadoAtual);

const botaoGhost = obterElementoPorIds('botaoModoFantasma', 'ghostmode');
botaoGhost?.addEventListener('click', () => {
    const estadoAtual = localStorage.getItem('ghostmode');
    const estadoNormalize = estadosGhostMode.includes(estadoAtual) ? estadoAtual : 'off';
    const indiceAtual = estadosGhostMode.indexOf(estadoNormalize);
    const proximoEstado = estadosGhostMode[(indiceAtual + 1) % estadosGhostMode.length];

    localStorage.setItem('ghostmode', proximoEstado);
    aplicarModoFantasma(proximoEstado);
});
